...
a fix reaches you without editing anything. `@v0.5.0` pins one exact release and
never changes.
...
- The **rule id** (`DataRaceGlobal`, `MissingJoin`, …), shown on the alert and in
  the SARIF.
- The **version**: `docker run --rm ghcr.io/coretrace/coretrace-concurrency-analyzer:v0.5.0 --version`.
