...
invalidates it. `--no-cache` recompiles everything and neither reads nor writes the cache.
...
The analysis parses a unit when it gets to it and lets the module go once its facts are
extracted, so the number of modules in memory is bounded by the number of workers —
`--max-live-units=N`, one per hardware thread by default — rather than by the size of the
project. With the cache, the bitcode is not held either: a unit is read from its cache entry
each time it is analysed, and the bytes go with its module. Without the cache (`--no-cache`)
nothing is written to disk, so the bitcode of every unit is held in memory for the whole run.

A unit the second pass has to revisit sees exactly the IR the first pass saw: from memory, it
is the same bytes; from the cache, the entry is read again and checked against the size and
digest recorded when the unit was added, and a unit whose entry changed in between is
reported as failed rather than analysed on different IR.
...
- **Without the cache, bitcode is held in memory for the whole run.** Modules are bounded; the
  bitcode they are parsed from is not, and grows with the project (138 MB for the 49-unit
  workload in `performance.md`). `--no-cache` writes nothing to disk on purpose, so it keeps
  that floor; with the cache, the floor is gone.
